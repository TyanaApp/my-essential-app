...
const typeColors: Record<EventType, string> = {
  trigger: 'bg-orange-500/20 text-orange-500 dark:text-orange-400 border-orange-500/30',
  symptom: 'bg-red-500/20 text-red-500 dark:text-red-400 border-red-500/30',
  cycle: 'bg-pink-500/20 text-pink-500 dark:text-pink-400 border-pink-500/30',
  medication: 'bg-blue-500/20 text-blue-500 dark:text-blue-400 border-blue-500/30',
  appointment: 'bg-green-500/20 text-green-500 dark:text-green-400 border-green-500/30',
  goal: 'bg-purple-500/20 text-purple-500 dark:text-purple-400 border-purple-500/30',
  intervention: 'bg-cyan-500/20 text-cyan-500 dark:text-cyan-400 border-cyan-500/30',
  outcome: 'bg-emerald-500/20 text-emerald-500 dark:text-emerald-400 border-emerald-500/30',
};
...
const ConfidenceIcon = ({ confidence }: { confidence?: 'high' | 'medium' | 'low' }) => {
  if (confidence === 'high') return <CheckCircle className="w-3 h-3 text-success" />;
  if (confidence === 'medium') return <HelpCircle className="w-3 h-3 text-warning" />;
  if (confidence === 'low') return <AlertCircle className="w-3 h-3 text-orange-500" />;
  return null;
};
...
const ImpactTrend = ({ impact }: { impact?: string }) => {
  ...
  return (
    <div className="flex items-center gap-1 text-[10px] text-muted-foreground">
      {isUp && <TrendingUp className="w-3 h-3 text-destructive" />}
      {isDown && <TrendingDown className="w-3 h-3 text-info" />}
...
const TimelineEvent: React.FC<TimelineEventProps> = ({
  ...
  return (
    <div className={`relative ${side === 'right' ? 'ml-auto' : 'mr-auto'}`}>
      ...
      >
        <div className="relative p-3 rounded-2xl shadow-lg w-[140px] transition-shadow hover:shadow-xl bg-card border border-border">
          {isImportant && (
          ...
          <div className="flex flex-col items-center text-center gap-0.5">
            ...
            <h3 className="font-semibold text-card-foreground text-xs leading-tight line-clamp-2">{title}</h3>

            <span className={`text-[9px] px-1.5 py-0.5 rounded-full border ${typeColors[type]}`}>
              {typeLabels[type]}
            </span>

            <span className="text-lg font-bold text-card-foreground mt-1">{date}</span>
